...
msgQueueInfoHQS_t * msgQueueInfoArr[taskHQSQty] = {NULL};
void msgOsHQSInit(void)
{
	int i;
	for (i = 0; i<taskHQSQty; i++)
	{
		*(msgQueueInfoArr + i) = NULL;
	}
}
...
void msgOsQueueInfoHQSCreate(uint32 TaskID, FillModeHQS_m fillMode, uint8 fluidic, FluidicModeHQS_m fluidicMode)
{
	...
	*(msgQueueInfoArr + TaskID)  = msgQueueAdd;
}
...
Msg_Err_m msgOsSendHQS(uint32 TaskID, uint16 dataSize,void * dataAdd)
{
	...
	uint8 count;
	msgQueueInfoHQS_t  *  msgQueueName =  *(msgQueueInfoArr + TaskID);
	...
	if (*(msgQueueInfoArr + TaskID) == NULL)
	{
	...
	OsHQSSetEvent(TaskID, OSMgsHQS_Event);
	return err;
}
...
void * msgOsReceive(uint32 TaskID)
{
	...
	void * dataAdd=NULL;
	msgPack = msgOSPakcReceive(msgQueueName);
	if (msgPack)
	{
